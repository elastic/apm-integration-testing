...
DEFAULT_COMPOSE_ARGS="${ELASTIC_STACK_VERSION} ${BUILD_OPTS} --no-apm-server-dashboards --no-apm-server-self-instrument --with-agent-python-django --with-agent-python-flask --agent-config-poll 1s --force-build --no-xpack-secure"
