...
def run_process(lang):
    if lang == 'node':
        ...
        cmd = ['gunicorn', '-w', '4', '-b', '0.0.0.0:5000', '--chdir', 'tests/agent/backend/python/flask', 'app:app']
...
def check_counts(elasticsearch, size, it):
    count = size * it
...
def check_contents(elasticsearch, lang, it):
    ...
    for hit in lookup(es.search(index, body=transactions_query), 'hits', 'hits'):
        ...
        assert lookup(context, 'app', 'name') == 'flask_app', context
...
def check_contents_not_mixed(elasticsearch):
...
def test_blackbox(apm_server, elasticsearch):
    ...
    for lang in langs:
        ...
        for it in range(1, iters + 1):
            ...
            load_test(lang, size)
            check_counts(elasticsearch, size, it)
            check_contents(elasticsearch, lang, it)
    ...
    if len(langs) > 1:
        ...
        for it in range(1, iters + 1):
            ...
            check_counts(elasticsearch, size * 2, it)
            check_contents_not_mixed(elasticsearch)
            logger.info("So far so good...")
